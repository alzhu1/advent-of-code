...
group_max_weight = int(total_weight / 4)
    ...
    for subset in subset_sum_dict[i]:
        ...
        for j in range(i, len(remaining)):
            ...
            for remaining_subset in remaining_subsets:
                ...
                for k in range(i, len(rem)):
                    remaining_subsets_k = list(itertools.combinations(tuple(rem), k))

                    for rem_sub_k in remaining_subsets_k:
                        if rem_sub_k not in subset_sums:
                            continue

                        r = rem[:]
                        for num in rem_sub_k:
                            r.remove(num)

                        if tuple(r) not in subset_sums:
                            continue
